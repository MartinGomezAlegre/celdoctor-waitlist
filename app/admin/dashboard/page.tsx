        ...
        <div className="flex min-h-screen bg-slate-100">
            ...
            <aside className="w-60 bg-[#1e0b4b] flex flex-col shrink-0">
                ...
                <nav className="flex-1 px-3 py-4 space-y-1">
                    {(
                        ...
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${section === id
                ...
                <div className="px-3 py-4 border-t border-white/10">
                    <button
                        onClick={logout}
                        className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium text-white/60 hover:bg-white/8 hover:text-white transition-colors"
                    >
                        <LogOut size={16} />
                        Cerrar sesión
                    </button>
...
function SectionDashboard({ token }: { token: string }) {
    ...
    useEffect(() => {
        fetch(getApiUrl("/admin/dashboard"), { headers: authHeaders(token) })
            .then((r) => r.json())
            .then((d) => setMetrics(d))
            .catch(() => null)
            .finally(() => setLoadingMetrics(false));

        fetch(getApiUrl("/admin/metricas-grafico"), { headers: authHeaders(token) })
            .then((r) => r.json())
            .then((d: GraficoPoint[]) => setGrafico(d))
            .catch(() => null)
    ...
    async function exportarExcel() {
        setExporting(true);
        try {
            const res = await fetch(getApiUrl("/admin/exportar-excel"), {
    ...
    const kpis = [
        ...
        { label: "Suscriptores activos", value: metrics?.suscriptores_activos?.toString() ?? null, Icon: Users, color: "text-emerald-600" },
        { label: "Nuevos hoy", value: metrics?.nuevos_hoy?.toString() ?? null, Icon: CreditCard, color: "text-blue-600" },
                    ...
                    <ResponsiveContainer width="100%" height={260}>
                        <LineChart data={grafico}>
                            ...
                            <Tooltip
                                ...
                                formatter={(value) => [value ?? 0, "Nuevas"]}
                            />
...
function SectionUsuarios({ token }: { token: string }) {
    ...
    useEffect(() => {
        fetch(getApiUrl("/admin/usuarios"), { headers: authHeaders(token) })
            .then((r) => r.json())
            .then((d: AdminUsuario[]) => setUsuarios(d))
            .catch(() => null)
...
function SectionSuscripciones({ token }: { token: string }) {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        fetch(getApiUrl(`/admin/suscripciones${qs}`), { headers: authHeaders(token) })
            .then((r) => r.json())
            .then((d: AdminSuscripcion[]) => setSuscripciones(d))
            .catch(() => null)
            .finally(() => setLoading(false));
    }, [token, filtroEstado]);
...
function SectionPlanes({ token }: { token: string }) {
    ...
    const fetchPlanes = useCallback(async () => {
        setLoading(true);
        const r = await fetch(getApiUrl("/planes"));
        const d: AdminPlan[] = await r.json().catch(() => []);
        setPlanes(d);
        const precios: Record<number, string> = {};
        d.forEach((p) => { precios[p.id] = p.precio_mensual.toString(); });
    ...
    async function guardarCambio(id: number, activo: boolean, precio: number) {
        setSaving(id);
        try {
            await fetch(getApiUrl(`/admin/planes/${id}`), {
                method: "PUT",
    ...
    return (
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {planes.map((plan) => {
                        const precioEditable = editPrecio[plan.id] ?? plan.precio_mensual.toString();
                        ...
                        return (
                            <div key={plan.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
                                ...
                                <div className={`text-xs font-semibold px-2.5 py-1 rounded-full inline-block ${plan.activo ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
