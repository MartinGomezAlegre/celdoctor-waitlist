...
import { TrendingUp, Users, Building2, XCircle, Package } from "lucide-react"
...
export default function SectionReportes({ token, addToast }: Props) {
    ...
    useEffect(() => {
        Promise.all([
            fetch(`${API}${adminEndpoints.reporteMensual}`, { headers: authHeaders(token) })
                .then((r) => r.json())
                .then((d: ReporteMensual) => setReporte(d))
                .catch(() => addToast("Error al cargar reporte mensual", "error")),
            fetch(`${API}${adminEndpoints.metricasEmbudo}`, { headers: authHeaders(token) })
                .then((r) => r.json())
                .then((d: MetricasEmbudo) => setEmbudo(d))
                .catch(() => setEmbudo(null)),
            fetch(`${API}${adminEndpoints.metricasRetencion}`, { headers: authHeaders(token) })
                .then((r) => r.json())
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <KpiCard
                        ...
                        sub={reporte ? `vs ${fmtCurrency(reporte.mrr_mes_anterior)} el mes pasado` : undefined}
                        loading={loading}
                    />
                    <KpiCard
                        ...
                        sub={reporte ? `vs ${reporte.nuevas_mes_anterior} el mes pasado` : undefined}
                        loading={loading}
                    />
                    <KpiCard
                        label="Cancelaciones"
                        ...
                        Icon={XCircle}
                        color="text-red-500"
                        sub={reporte ? `vs ${reporte.cancelaciones_mes_anterior} el mes pasado` : undefined}
                        highlight={(reporte?.cancelaciones ?? 0) > (reporte?.cancelaciones_mes_anterior ?? 0)}
                        loading={loading}
                    />
                    <KpiCard
                    ...
                    {reporte?.top_plan && (
                        <KpiCard
                            label={`Top plan: ${reporte.top_plan}`}
                            value="—"
                            Icon={Package}
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col sm:flex-row gap-0 sm:gap-0 overflow-hidden rounded-xl">
                        {[
                            { label: "Registrados", value: embudo.visitantes_registrados, color: "bg-violet-100 text-violet-700", width: "100%" },
                            { label: "Iniciaron checkout", value: embudo.iniciaron_checkout, color: "bg-violet-200 text-violet-800", width: embudo.visitantes_registrados ? `${Math.round((embudo.iniciaron_checkout / embudo.visitantes_registrados) * 100)}%` : "0%" },
                            { label: "Completaron pago", value: embudo.completaron_pago, color: "bg-violet-600 text-white", width: embudo.visitantes_registrados ? `${Math.round((embudo.completaron_pago / embudo.visitantes_registrados) * 100)}%` : "0%" },
                        ].map((step) => (
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                    ...
                    <ResponsiveContainer width="100%" height={240}>
                        <BarChart data={retencion}>
                            ...
                            <Bar dataKey="activos_al_mes_siguiente" name="Retenidos" fill="#8b5cf6" radius={[4, 4, 0, 0]} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            )}
